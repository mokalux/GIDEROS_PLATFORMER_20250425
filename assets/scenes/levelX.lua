...
local ispaused = false -- with the letter 'p'
...
function LevelX:init()
	...
	self.tiledlevels = {}
	self.tiledlevels[1] = "tiled/lvl001/level" -- lua file without extension
	self.tiledlevels[2] = "tiled/lvl001/levelB" -- lua file without extension
--	self.tiledlevels[3] = "tiled/lvl001/levelC" -- lua file without extension
	local mapdef = {} -- game area (rect: top, left, right, bottom)
	local zoom = 1.7 -- 1.8, 1.6, 2, 1.8
	local camfollowoffsety = 32 -- 36, 48, camera follow player1 y offset, magik XXX
	self.tiny.player1inventory = {} -- player1 inventory (keys, ...)
	self.tiny.numberofcoins = 0 -- can be 'score'
	...
	mapdef.t, mapdef.l = currlevel.mapdef.t, currlevel.mapdef.l
	mapdef.r, mapdef.b = currlevel.mapdef.l+currlevel.mapdef.r, currlevel.mapdef.t+currlevel.mapdef.b
	...
	self.camera = GCam.new(layers["main"]) -- (content [, anchorX, anchorY]) -- anchor default 0.5, 0.5
	...
	self.camera:setPrediction(1) -- 0.8, 0.75, 0.5, number betwwen 0 and 1
...
--	 	SDebugShield.new(self.tiny),
		...
		SAI_Npc.new(self.tiny, self.player1),
		SMvpf.new(self.tiny, bworld),
		SDoorSensor.new(self.tiny, bworld),
...
--		SHideable.new(self.tiny, self.player1)
	)
...
function LevelX:onEnterFrame(e)
	if not ispaused then
		...
		if self.player1.restart then
			if g_currlevel > #self.tiledlevels then -- win scene
				timer -= 1
				if self.camera:getZoom() < 3 then
